      ...
      <section id="content">
        ...
        {Object.values(this.props.posts)
          .sort((post_a, post_b) => {
            ...
            else return post_a.voteScore - post_b.voteScore;
          })
          .reverse(post => post)
          .map(post => (
            <article className="post" key={post.id}>
              ...
              <p className="post-meta">
                <span className="post-author">
                  Posted on{" "}
                  {new Date(post.timestamp).toLocaleDateString("en-US")} by{" "}
                  {post.author}
                </span>
              </p>
...
const mapDispatchToProps = dispatch => ({
  ...
  vote: (postID, voteString) => dispatch(voteOnPost(postID, voteString)),
